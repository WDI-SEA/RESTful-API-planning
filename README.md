...
| HTTP METHOD (_Verb_) | URL (_Nouns_)                    | CRUD   | Response                                     | Notes |
| -------------------- | -------------------------------- | ------ | -------------------------------------------- | ----- |
| Post                 | /members/new                     | Create | Create a new member                          |       |
| Put                  | /members                         | Update | Update the members list with the new member  |       |
| Get                  | /members/books/:id/:user_id      | Read   | Read members current checked out books       |       |
| Get                  | /books                           | Read   | Get array of all books                       |       |
| Get                  | /books_genres/:book_id/:genre_id | Read   | Get genres of that book by its book_id       |       |
| Post                 | /books/new                       | Create | Create a new book with an id                 |       |
| Put                  | /books                           | Update | Update the books data base with the new book |       |
| Get                  | /genres                          | Read   | See all genres available                     |       |
| Post                 | /genres/new                      | Create | Create a new genres not in the current DB    |       |
| Put                  | /genres                          | Update | Update genres DB with the new genre          |       |
| Delete               | /members/:id                     | Delete | Delete current user from database            |       |
| Delete               | /books/:id                       | Delete | Delete current book from database            |       |
| Delete               | /genres/:id                      | Delete | Delete current genre from database           |       |
|                      |                                  |        |                                              |       |
